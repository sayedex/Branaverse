...
export const UnStakeWindrow = forwardRef(({pool}:Props,ref: any) => {
  const {address} = useAccount()
    //costom hook for stake fund...
    const dispatch = useAppdispatch();
    const [value,setvalue] = useState("1")
    const [open, setOpen] = useState(false);
    const {writeAsync:StakeToken,isLoading,isError,isFetching,writeLoading,data,status} = useSendtrsansation(pool,dispatch,"withdraw",value?value:"1","withdraw",address)
    const handleChange = ()=>{
      console.log("as");
      //pool,dispatch,"deposit",value?value:"1","deposit",address)
    }
...
<input type="text" value={value} onChange={(e)=>setvalue(e.target.value)}  id="first_name" className="
...
<span className='pr-3 font-semibold'>{pool.Staketoken.symbol}</span>

</div>
{/* balance  */}
<div>
<span className='text-sm text-gray-400 font-normal'>Staked : {Math.floor(pool.staked?pool.staked:0)}</span>
...
{/* confirm button */}
    ...
    UnStakeWindrow.displayName = 'StakeWindrow';